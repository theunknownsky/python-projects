...
from backend.pay import Pay
import backend.errorDialogs as ed
Form, Window = uic.loadUiType("frontend/landing_page.ui")
...
class Home(QMainWindow):
    def __init__(self):
        ...
        self.ui.discBtn.clicked.connect(self.applyDiscount)
    ...
    def increaseTotal(self, price):
        self.total += price
        self.ui.totalShow.setText(f"Total Price: {self.total}")
        if self.discountToggle:
            self.discountedPrice = self.total * 0.8
            self.ui.discTotalShow.setText(f"Discounted Price: {self.discountedPrice}")
    ...
    def applyDiscount(self):
        if self.account.age >= 65 and not self.discountToggle:
            self.discountedPrice = self.total * 0.8
            self.ui.discTotalShow.setText(f"Discounted Price: {self.discountedPrice}")
            self.discountToggle = True
        elif self.account.age >= 65 and self.discountToggle:
            pass
        else:
            self.ui.discTotalShow.setText(f"Discounted Price: Not eligible")
